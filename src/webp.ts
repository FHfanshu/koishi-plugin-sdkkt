...
import { binarySearchForMetadata } from './jpeg';
 ...
 * Supports EXIF, XMP chunks and binary search
 */
export function parseWebPMetadata(buffer: Buffer, logger?: Logger): MetadataResult {
    try {
 ...
 */
function parseWebPChunks(buffer: Buffer): MetadataResult {
    try {
        const metadata: SDMetadata = {};
...
function extractWebPChunkText(chunkData: Buffer): string {
    ...
    for (const encoding of encodings) {
        try {
            let text = chunkData.toString(encoding);
