...
import defaultExifReader from 'exifreader';
import { parseA1111Parameters, SDMetadata } from './a1111';
import { extractNovelAIMetadata } from './novelai';
import { extractComfyUIMetadata, isComfyUIFormat } from './comfyui';

const exifreader = defaultExifReader;
 ...
 * Supports APP segments and EXIF data
 */
export function parseJPEGMetadata(buffer: Buffer, logger?: Logger): MetadataResult {
    try {
        ...
        if (!metadata.parameters) {
            try {
                ...
                if (exif) {
                    ...
                    for (const [fieldName, field] of Object.entries(exif)) {
                        try {
                            // Skip if already checked the standard fields
                            if (exifFields.includes(fieldName)) continue;

                            const fieldAny = field as { description?: string; value?: unknown; text?: string };
                ...
                if (!metadata.parameters) {
                    ...
                    for (const fieldName of exifFields) {
                        const field = (tags.exif as Record<string, unknown>)?.[fieldName];
                        if (field) {
                            // Collect field for fallback
                            exifData[fieldName] = field;
                            const fieldAny = field as { description?: string; value?: unknown };
                            const fieldValue = fieldAny.description || fieldAny.value;
                            if (fieldValue && typeof fieldValue === 'string') {
                                if (fieldValue.includes('Steps:') || fieldValue.includes('Sampler:') ||
                                    fieldValue.includes('CFG scale:') || fieldValue.includes('Seed:')) {
                                    const lines = fieldValue.split('\n').slice(0, 2).join('\n');
                ...
                if (!metadata.parameters && (tags.exif as Record<string, unknown>)?.Software) {
                    const software = ((tags.exif as Record<string, unknown>).Software as { description?: string; value?: unknown }).description || ((tags.exif as Record<string, unknown>).Software as { description?: string; value?: unknown }).value;
                    if (software === 'NovelAI') {
                        const descField = (tags.exif as Record<string, unknown>)?.ImageDescription;
                        const description = (descField as { description?: string; value?: unknown })?.description || (descField as { description?: string; value?: unknown })?.value || undefined;
                        const commentField = (tags.exif as Record<string, unknown>)?.UserComment;
                        const comment = (commentField as { description?: string; value?: unknown })?.description || (commentField as { description?: string; value?: unknown })?.value || undefined;
                        if (comment) {
                            extractNovelAIMetadata(comment as string, description as string | undefined, metadata);
                            // Set parameters as a fallback display
                            metadata.parameters = comment as string;
                        }
                    }
                }
                ...
                if (!metadata.parameters && exifData) {
                    ...
                    for (const fieldName of comfyUIFields) {
                        const field = exifData[fieldName] || (tags.exif as Record<string, unknown>)?.[fieldName];
                        if (!field) continue;

                        const fieldValue = typeof field === 'string' ? field :
                            ((field as { description?: string; value?: unknown; text?: string }).description || (field as { description?: string; value?: unknown; text?: string }).value || (field as { description?: string; value?: unknown; text?: string }).text);
                ...
                if (tags.xmp && !metadata.parameters) {
                    ...
                    try {
                        ...
                        for (const xmpField of xmpFields) {
                            const xmpDesc = (tags.xmp as Record<string, unknown>)[xmpField];
                            if (xmpDesc) {
                                const descValue = typeof xmpDesc === 'string' ? xmpDesc : ((xmpDesc as { value?: unknown; description?: unknown }).value || (xmpDesc as { value?: unknown; description?: unknown }).description);
                ...
                if (tags.iptc && !metadata.parameters) {
                    ...
                    const iptcCaption = (tags.iptc as Record<string, unknown>)['Caption/Abstract'];
                    if (iptcCaption) {
                        const captionValue = typeof iptcCaption === 'string' ? iptcCaption : ((iptcCaption as { value?: unknown; description?: unknown }).value || (iptcCaption as { value?: unknown; description?: unknown }).description);
                        if (captionValue && typeof captionValue === 'string' && captionValue.includes('Steps:')) {
 ...
 */
function extractJpegAppSegments(buffer: Buffer): Record<string, string> {
...
function extractSegmentText(segmentData: Buffer): string {
    ...
    for (const encoding of encodings) {
        try {
            const text = segmentData.toString(encoding);
 ...
 */
export function binarySearchForMetadata(buffer: Buffer, format: string = 'jpeg'): string | null {
    const minLength = 50;
        ...
        try {
            ...
            for (const pattern of patterns) {
                ...
                if (matches && matches.length > 0) {
                    ...
                    if (candidate.includes('Steps:') && (candidate.includes('Sampler:') ||
