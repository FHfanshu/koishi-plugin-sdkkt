 ...
 * Supports tEXt, iTXt, zTXt chunks and Stealth PNG
 */
export function parsePNGMetadata(buffer: Buffer, logger?: { info: (msg: string, ...args: unknown[]) => void }): MetadataResult {
    try {
        // Extract text chunks
 ...
 */
function parseStealthPNGMetadata(buffer: Buffer): MetadataResult {
    try {
        ...
        return {
            success: false,
            error: `Failed to parse Stealth PNG: ${(error as Error).message}`
        };
    }
}
 ...
 */
function parseTextChunk(chunkData: Buffer, textChunks: Record<string, string>): void {
 ...
 */
function parseInternationalTextChunk(chunkData: Buffer, textChunks: Record<string, string>): void {
    const nullIndex = chunkData.indexOf(0);
 ...
 */
function parseCompressedTextChunk(chunkData: Buffer, textChunks: Record<string, string>): void {
    const nullIndex = chunkData.indexOf(0);
    if (nullIndex === -1) return;
