 ...
 */
export function decodeDataUri(value: unknown): Buffer | null {
 ...
 */
export function bufferFromBase64(value: unknown): Buffer | null {
    if (typeof value !== 'string') return null;
 ...
 */
export function detectImageFormat(buffer: Buffer): 'png' | 'jpeg' | 'webp' | null {
 ...
 */
export function normalizeLocalPath(target: unknown): string | null {
 ...
 */
export async function tryReadLocalFileBuffer(target: unknown): Promise<Buffer | null> {
 ...
 */
export function guessFileExtension(attrs: Record<string, unknown>, sourceHint?: string): string {
    const candidates = [
        attrs?.file,
        attrs?.filename,
        attrs?.name,
 ...
 */
export function makeImageSegmentKey(segment: { attrs?: Record<string, unknown>; data?: Record<string, unknown> }): string {
    ...
    // Try URL
    const url = normalizeUrl(a.src as string || a.url as string || d.src as string || d.url as string);
    if (url) return url;
 ...
 */
export function parseMetadataValue<T>(value: unknown, parser: (v: unknown) => T | null): T | undefined {
