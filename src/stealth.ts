 ...
 */
export function extractStealthPngMetadata(png: PNGData): SDMetadata | null {
    try {
        ...
        const lowestBits = new Array(totalAlphaBits);
        let bitIdx = 0;
 ...
 */
export function hasStealthMetadata(png: PNGData): boolean {
    try {
 ...
 */
export function getStealthMetadataSize(png: PNGData): number | null {
    try {
        const { width, height, data: pngData } = png;
        if (!pngData || pngData.length === 0) {
            return null;
