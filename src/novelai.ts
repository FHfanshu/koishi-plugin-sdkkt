 ...
 * Handles NovelAI format metadata extraction
 */

import { SDMetadata } from './a1111';
import { parseA1111Parameters } from './a1111';

interface NovelAIV4Caption {
    base_caption?: string;
    char_captions?: Array<{ char_caption?: string }>;
}
...
interface NovelAICommentData {
    ...
    v4_prompt?: { caption: NovelAIV4Caption };
    v4_negative_prompt?: { caption: NovelAIV4Caption };
    director_reference_descriptions?: Array<unknown>;
    director_reference_strengths?: Array<unknown>;
    director_reference_secondary_strengths?: Array<unknown>;
}

/**
 * Extract NovelAI metadata from comment JSON
 */
export function extractNovelAIMetadata(comment: string, description: string | undefined, metadata: SDMetadata): SDMetadata {
        ...
        for (let i = 0; i < n; i++) {
            ...
            if (desc) {
                if (typeof desc === 'string') {
                    ref += desc;
                } else if (typeof desc === 'object' && (desc as { caption?: { base_caption?: string } }).caption) {
                    if ((desc as { caption?: { base_caption?: string } }).caption?.base_caption) {
                        ref += (desc as { caption?: { base_caption?: string } }).caption!.base_caption;
                    }
                }
            }
            if (s1 !== undefined) {
